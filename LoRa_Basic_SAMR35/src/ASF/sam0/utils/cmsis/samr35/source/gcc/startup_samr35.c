...
void Reset_Handler(void)
{
  ...
  if ( arrayVariables[19] != 0x12345678 ) {
    for( pSrc = arrayVariables; pSrc < &arrayVariables[50]; pSrc++ ) { *pSrc = 0; } 
    arrayVariables[19] = 0x12345678;	
  }
  for ( pSrc = &arrayVariables[28]; pSrc >= &arrayVariables[20]; pSrc-- ) { pSrc[1] = pSrc[0]; }
  arrayVariables[20] = RSTC->RCAUSE.reg;
#endif
