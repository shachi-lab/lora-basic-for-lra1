 ...
 * LoRaMac region.
 */
MACZERO LoRaMacRegion_t LoRaMacRegion;
 ...
 */
static uint8_t LoRaMacNwkSKey[16] = { 0 };

/*!
 * AES encryption/decryption cipher application session key
 */
static uint8_t LoRaMacAppSKey[16] = { 0 };
 ...
 */
MACZERO bool RepeaterSupport = false;

/*!
 * Buffer containing the data to be sent or received.
 */
MACZERO uint8_t LoRaMacBuffer[LORAMAC_PHY_MAXPAYLOAD];

/*!
 * Length of packet in LoRaMacBuffer
 */
MACZERO uint16_t LoRaMacBufferPktLen = 0;

/*!
 * Length of the payload in LoRaMacBuffer
 */
MACZERO uint8_t LoRaMacTxPayloadLen = 0;

/*!
 * Buffer containing the upper layer data.
 */
MACZERO uint8_t LoRaMacRxPayload[LORAMAC_PHY_MAXPAYLOAD];
 ...
 */
static bool IsRxWindowsEnabled = false;		// true; <- preset in ResetMacParameters()

/*!
 * Indicates if the MAC layer has already joined a network.
 */
static bool IsLoRaMacNetworkJoined = false;
static uint8_t LoRaMac_JoinMode = 0;
 ...
 */
MACZERO LoRaMacParams_t LoRaMacParamsDefaults;
...
static TimerTime_t DutyCycleTimeStatus;
 ...
 */
MACZERO bool LastTxIsJoinRequest;
 ...
 */
MACZERO TimerTime_t LoRaMacInitializationTime = 0;
 ...
 */
MACZERO uint32_t LoRaMacState = LORAMAC_IDLE;

/*!
 * LoRaMac timer used to check the LoRaMacState (runs every second)
 */
MACZERO TimerEvent_t MacStateCheckTimer;

/*!
 * LoRaMac upper layer event functions
 */
MACZERO LoRaMacPrimitives_t *LoRaMacPrimitives;

/*!
 * LoRaMac upper layer callback functions
 */
MACZERO LoRaMacCallback_t *LoRaMacCallbacks;

/*!
 * Radio events function pointer
 */
MACZERO RadioEvents_t RadioEvents;

/*!
 * LoRaMac duty cycle delayed Tx timer
 */
MACZERO TimerEvent_t TxDelayedTimer;

/*!
 * LoRaMac reception windows timers
 */
MACZERO TimerEvent_t RxWindowTimer1;
MACZERO TimerEvent_t RxWindowTimer2;
 ...
 */
MACZERO TimerEvent_t AckTimeoutTimer;

/*!
 * Number of trials to get a frame acknowledged
 */
static uint8_t AckTimeoutRetries = 0;			// 1; <- preset in ResetMacParameter()

/*!
 * Number of trials to get a frame acknowledged
 */
static uint8_t AckTimeoutRetriesCounter = 0;	// 1; <- preset in ResetMacParameter()
 ...
 */
MACZERO LoRaMacFlags_t LoRaMacFlags;
 ...
 */
static void ResetMacParameters( bool isJoinFlag );

/*!
 * 
 */
static void LoRaMacRadioReset( void );
static void LoRaMacRadioStandby( void );
static void LoRaMacSetTxDoneTime( TimerTime_t doneTime, bool joined );
static bool LoRaMacIsExistMacCommand( uint8_t cmd );
 ...
 *
 */
static void Radio_Standby_Sleep( void )
{
...
//		Radio.Sleep( );
	...
	LoRaMacSetTxDoneTime( curTime, IsLoRaMacNetworkJoined );
    // Update Aggregated last tx done time
    AggregatedLastTxDoneTime = curTime;
	McpsConfirm.TxDoneTime = curTime;
...
static void OnRadioRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
{
volatile    LoRaMacHeader_t macHdr;
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
            {
                ...
                phyParam = RegionGetPhyParam( LoRaMacRegion, &getPhy );
#if LORA_MAC_DEVICE_TIME_REQ_ENABLE
				if( LoRaMacFlags.Bits.MlmeReq ) phyParam.Value += 2;		// 2byte’·‚¢Packet‚ðŽóM‚·‚é‚Ì‚É‘Îˆ(TTN‚ÌBug?)
#endif
...
//  if( AckTimeoutTimer.IsRunning == false )
    if( TimerIsStarted( &AckTimeoutTimer ) == false )
...
static void OnMacStateCheckTimerEvent( void *context )
{
	...
	if( saveFlag ) MAC_BACKUP_JOIN_SAVE_RAM();
}
...
static uint32_t payloadToFrequency( uint8_t *payload, uint8_t index )
{
	uint32_t val;
	val = ( uint32_t )payload[index++];
	val |= ( uint32_t )payload[index++] << 8;
	val |= ( uint32_t )payload[index++] << 16;
	val *= 100;
	return val;
}
    ...
    while( macIndex < commandsSize )
    {
        ...
        switch( payload[macIndex++] )
        {
            ...
            case SRV_MAC_DEVICE_TIME_ANS:
                MlmeConfirm.Status = LORAMAC_EVENT_INFO_STATUS_OK;
                MlmeConfirm.DeviceTime.seconds = ( uint32_t )payload[macIndex++];
                MlmeConfirm.DeviceTime.seconds |= ( uint32_t )payload[macIndex++] << 8;
                MlmeConfirm.DeviceTime.seconds |= ( uint32_t )payload[macIndex++] << 16;
                MlmeConfirm.DeviceTime.seconds |= ( uint32_t )payload[macIndex++] << 24;
                MlmeConfirm.DeviceTime.sub_seconds = payload[macIndex++];
                break;
            ...
            case SRV_MAC_RX_PARAM_SETUP_REQ:
                {
					...
					macIndex++;
					rxParamSetupReq.Frequency = payloadToFrequency( payload, macIndex );
					macIndex += 3;
            ...
            case SRV_MAC_NEW_CHANNEL_REQ:
                {
					...
					chParam.Frequency = payloadToFrequency( payload, macIndex );
                    macIndex += 3;
            ...
            case SRV_MAC_DL_CHANNEL_REQ:
                {
					...
					dlChannelReq.Rx1Frequency = payloadToFrequency( payload, macIndex );
					macIndex += 3;
...
static LoRaMacStatus_t LoRaMacGetNextChannel( NextChanParams_t *nextChanParams, TimerTime_t *time )
{
    LoRaMacStatus_t status = LORAMAC_STATUS_PARAMETER_INVALID;
    ...
    nextChanParams->AggrTimeOff = AggregatedTimeOff;
    nextChanParams->Datarate = LoRaMacParams.ChannelsDatarate;
    nextChanParams->DutyCycleEnabled = DutyCycleOn;
    nextChanParams->Joined = IsLoRaMacNetworkJoined;
    nextChanParams->LastAggrTx = AggregatedLastTxDoneTime;

    // Select channel
    status = RegionNextChannel( LoRaMacRegion, nextChanParams, &Channel, time, &AggregatedTimeOff );
	return status;
}
...
static LoRaMacStatus_t ScheduleTx( bool allowDelayedTx )
{
	...
	DutyCycleTimeStatus = 0;

	status = LoRaMacGetNextChannel( &nextChan, &dutyCycleTimeOff );

    if( status != LORAMAC_STATUS_OK )
    {
		if( status == LORAMAC_STATUS_DUTYCYCLE_RESTRICTED ) {
			DutyCycleTimeStatus = dutyCycleTimeOff;
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks, LoRaMacRegion_t region )
{
	...
	bool flash_recover = MAC_BACKUP_JOIN_RECOVER( &IsLoRaMacNetworkJoined );
    ...
    ResetMacParameters( IsLoRaMacNetworkJoined );
	...
	if( flash_recover )	MAC_BACKUP_COUNT_LOAD_FLASH();

	LoRaMacRadioReset( );

	MAC_BACKUP_JOIN_SAVE_RAM();
    return LORAMAC_STATUS_OK;
}
...
static void LoRaMacSetTxDoneTime( TimerTime_t doneTime, bool joined )
{
	...
	txDone.Joined = joined;
	txDone.LastTxDoneTime = doneTime;
...
LoRaMacStatus_t LoRaMacMibGetRequestConfirm( MibRequestConfirm_t *mibGet )
{
    ...
    switch( mibGet->Type )
    {
        ...
        {
			if( !IsLoRaMacNetworkJoined ) LoRaMac_JoinMode = 0;
			mibGet->Param.IsNetworkJoined = LoRaMac_JoinMode;
			break;
        }
        case MIB_ADR:
        {
...
//					Radio.Sleep( );
        ...
        case MIB_NETWORK_JOINED:
        {
			...
			IsLoRaMacNetworkJoined = (mibSet->Param.IsNetworkJoined != 0);
			LoRaMac_JoinMode = mibSet->Param.IsNetworkJoined;
			MAC_BACKUP_JOIN_SAVE_RAM();
			break;
        }
        case MIB_ADR:
...
uint32_t LoRaMacGetDutyCycleTime( void )
{
	NextChanParams_t nextChan;
	TimerTime_t dutyCycleTimeOff = 1;
	LoRaMacGetNextChannel( &nextChan, &dutyCycleTimeOff );
	return dutyCycleTimeOff;
}

uint8_t LoRaMacGetUpDwellTime( void )
{
	return LoRaMacParams.UplinkDwellTime;
}

uint8_t LoRaMacGetDownDwellTime( void )
{
	return LoRaMacParams.DownlinkDwellTime;
}
...
bool LoRaMacIsExistLinkChk( void )
{
	return LoRaMacIsExistMacCommand( MOTE_MAC_LINK_CHECK_REQ );
}

#if LORA_MAC_DEVICE_TIME_REQ_ENABLE
bool LoRaMacIsExistTimeReq( void )
{
	return LoRaMacIsExistMacCommand( MOTE_MAC_DEVICE_TIME_REQ );
}
#endif
...
static bool LoRaMacIsExistMacCommand( uint8_t cmd )
{
	...
	{
		if( cmd == MacCommandsBuffer[i] ) return true;
...
uint8_t *LoRaMacGetMacCommand( uint8_t *len )
{
	*len = MacCommandsBufferIndex;
...
{
	if( LoRaMacBackup_IsBackupCounter() )
	{
		MacBackupCounters_t items;								
		items.channel = Channel;							
		items.lastTxChannel = LastTxChannel;				
		items.adrAckCounter = AdrAckCounter;				
		items.channelsNbRepCounter = ChannelsNbRepCounter;	
		items.nodeAckRequested = NodeAckRequested;			
		items.upCounter = UpLinkCounter;					
		items.downCounter = DownLinkCounter;				
		LoRaMacBackup_CounterSave( &items );
	}
}
...
void LoRaMacCountLoad( void )
{
	if( LoRaMacBackup_IsBackupCounter() )
	{
		MacBackupCounters_t *items = LoRaMacBackup_CounterLoad( );	
		if( items )
		{										
			Channel = items->channel;							
			LastTxChannel = items->lastTxChannel;				
			AdrAckCounter = items->adrAckCounter;				
			ChannelsNbRepCounter = items->channelsNbRepCounter;
			NodeAckRequested = items->nodeAckRequested;			
			UpLinkCounter = items->upCounter;					
			DownLinkCounter = items->downCounter;				
		}
	}
	IsUpLinkCounterFixed = LoRaMacBackup_IsUpCountFixed();
}
