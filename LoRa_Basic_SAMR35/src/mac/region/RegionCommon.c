...
void RegionCommonComputeRxWindowParameters( double tSymbol, uint8_t minRxSymbols, uint32_t rxError, uint32_t wakeUpTime, uint32_t* windowTimeout, int32_t* windowOffset )
{
	...
	tmp = ceil( ( ( 2 * minRxSymbols - 8 ) * tSymbol + 2 * rxError ) / tSymbol );
	*windowTimeout = MAX( ( uint32_t )tmp, minRxSymbols );					// Computed number of symbols
	tmp = ceil( ( 4.0 * tSymbol ) - ( ( *windowTimeout * tSymbol ) / 2.0 ) - wakeUpTime );
	*windowOffset = ( int32_t )tmp;
}
...
int8_t RegionCommonComputeTxPower( int8_t txPowerIndex, float maxEirp, float antennaGain )
{
	...
	tmp = floor( ( maxEirp - ( txPowerIndex * 2U ) ) - antennaGain );
	phyTxPower = ( int8_t )tmp;
