...
static const Band_t BandsDefault[AS923_MAX_NB_BANDS] = { AS923_BAND0 };
...
void RegionAS923InitDefaults( InitType_t type )
{
    switch( type )
    {
        case INIT_TYPE_INIT:
        {
			...
			memcpy1( (uint8_t*)Bands, (uint8_t*)BandsDefault, sizeof(Bands) );
            break;
        }
        ...
        case INIT_TYPE_APP_DEFAULTS:
        {
			...
			for( uint8_t i = 0; i < AS923_MAX_NB_CHANNELS; i++ ) {
				if( (ChannelsMask[0] & (1 << i)) == 0 ) Channels[i].Frequency = 0;
			}
            break;
        }
        default:
